...
import { GizmoDefaultError } from "../models/errors/gizmodefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | GizmoDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | GizmoDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new GizmoDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new GizmoDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
